...
static inline void v4l2_m2m_exp_set_timeperframe(V4L2m2mContext *s, unsigned int num, unsigned int den)
{
...
static inline void v4l2_m2m_exp_set_ext_ctrl(V4L2m2mContext *s, unsigned int id, signed int value, const char *name, int log_warning)
{
...
static inline int v4l2_m2m_exp_get_ext_ctrl(V4L2m2mContext *s, unsigned int id, signed int *value, const char *name, int log_warning)
{
    struct v4l2_ext_controls ctrls = { { 0 } };
    struct v4l2_ext_control ctrl = { 0 };
    int ret;
...
static inline unsigned int v4l2_m2m_exp_h264_profile_from_ff(int p)
{
...
static inline int v4l2_m2m_exp_mpeg4_profile_from_ff(int p)
{
...
static int v4l2_m2m_exp_check_b_frame_support(V4L2m2mContext *s)
{
    if (s->avctx->max_b_frames)
        av_log(s->avctx, AV_LOG_WARNING, "Encoder does not support b-frames yet\n");

    v4l2_m2m_exp_set_ext_ctrl(s, MPEG_CID(B_FRAMES), 0, "number of B-frames", 0);
    v4l2_m2m_exp_get_ext_ctrl(s, MPEG_CID(B_FRAMES), &s->avctx->max_b_frames, "number of B-frames", 0);
...
static inline void v4l2_m2m_exp_subscribe_eos_event(V4L2m2mContext *s)
{
...
static int v4l2_m2m_exp_prepare_encoder(V4L2m2mContext *s)
{
     ...
     * requirements
     */
    v4l2_m2m_exp_subscribe_eos_event(s);

    ret = v4l2_m2m_exp_check_b_frame_support(s);
    if (ret)
        return ret;
    ...
    if (avctx->framerate.num || avctx->framerate.den)
        v4l2_m2m_exp_set_timeperframe(s, avctx->framerate.den, avctx->framerate.num);

    /* set ext ctrls */
    v4l2_m2m_exp_set_ext_ctrl(s, MPEG_CID(HEADER_MODE), MPEG_VIDEO(HEADER_MODE_SEPARATE), "header mode", 0);
    v4l2_m2m_exp_set_ext_ctrl(s, MPEG_CID(BITRATE) , avctx->bit_rate, "bit rate", 1);
    v4l2_m2m_exp_set_ext_ctrl(s, MPEG_CID(FRAME_RC_ENABLE), 1, "frame level rate control", 0);
    v4l2_m2m_exp_set_ext_ctrl(s, MPEG_CID(GOP_SIZE), avctx->gop_size,"gop size", 1);
    ...
    case AV_CODEC_ID_H264:
        if (avctx->profile != FF_PROFILE_UNKNOWN) {
            val = v4l2_m2m_exp_h264_profile_from_ff(avctx->profile);
            if (val < 0)
                av_log(avctx, AV_LOG_WARNING, "h264 profile not found\n");
            else
                v4l2_m2m_exp_set_ext_ctrl(s, MPEG_CID(H264_PROFILE), val, "h264 profile", 1);
        }
    ...
    case AV_CODEC_ID_MPEG4:
        if (avctx->profile != FF_PROFILE_UNKNOWN) {
            val = v4l2_m2m_exp_mpeg4_profile_from_ff(avctx->profile);
            if (val < 0)
                av_log(avctx, AV_LOG_WARNING, "mpeg4 profile not found\n");
            else
                v4l2_m2m_exp_set_ext_ctrl(s, MPEG_CID(MPEG4_PROFILE), val, "mpeg4 profile", 1);
        }
        qmin_cid = MPEG_CID(MPEG4_MIN_QP);
        qmax_cid = MPEG_CID(MPEG4_MAX_QP);
        if (avctx->flags & AV_CODEC_FLAG_QPEL)
            v4l2_m2m_exp_set_ext_ctrl(s, MPEG_CID(MPEG4_QPEL), 1, "qpel", 1);
        qmin = 1;
        qmax = 31;
        break;
    ...
    v4l2_m2m_exp_set_ext_ctrl(s, qmin_cid, qmin, "minimum video quantizer scale",
                      avctx->qmin >= 0);
    v4l2_m2m_exp_set_ext_ctrl(s, qmax_cid, qmax, "maximum video quantizer scale",
                      avctx->qmax >= 0);

    return 0;
}

static int v4l2_m2m_exp_send_frame(AVCodecContext *avctx, const AVFrame *frame)
{
    ...
    if (frame && frame->pict_type == AV_PICTURE_TYPE_I)
        v4l2_m2m_exp_set_ext_ctrl(s, MPEG_CID(FORCE_KEY_FRAME), 0, "force key frame", 1);
#endif

    return ff_v4l2_m2m_exp_context_enqueue_frame(output, frame);
}

static int v4l2_m2m_exp_receive_packet(AVCodecContext *avctx, AVPacket *avpkt)
{
    V4L2m2mContext *s = ((V4L2m2mPriv*)avctx->priv_data)->context;
    ...
    ret = v4l2_m2m_exp_send_frame(avctx, frame);
...
dequeue:
    return ff_v4l2_m2m_exp_context_dequeue_packet(capture, avpkt);
}

static av_cold int v4l2_m2m_exp_encode_init(AVCodecContext *avctx)
{
    ...
    return v4l2_m2m_exp_prepare_encoder(s);
}

static av_cold int v4l2_m2m_exp_encode_close(AVCodecContext *avctx)
{
...
static const AVCodecDefault v4l2_m2m_exp_m2m_defaults[] = {
    { "qmin", "-1" },
    { "qmax", "-1" },
    { NULL },
};
...
#define M2MENC(NAME, LONGNAME, OPTIONS_NAME, CODEC) \
    ...
    AVCodec ff_ ## NAME ## _v4l2m2m_exp_encoder = { \
        ...
        .init           = v4l2_m2m_exp_encode_init, \
        .receive_packet = v4l2_m2m_exp_receive_packet, \
        .close          = v4l2_m2m_exp_encode_close, \
        .defaults       = v4l2_m2m_exp_m2m_defaults, \
